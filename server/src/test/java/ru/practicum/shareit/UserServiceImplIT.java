...
class UserServiceImplIT {
    ...
    void create_shouldPersistAndReturnId() {
        UserDto created = userService.create(new UserDto(null, "Tony", "tony@test.com"));

        assertNotNull(created.getId());
        assertEquals("Tony", created.getName());
        assertEquals("tony@test.com", created.getEmail());

        UserDto loaded = userService.findById(created.getId());
        assertEquals(created.getId(), loaded.getId());
        assertEquals("Tony", loaded.getName());
    }
