...
public class BookingServiceImpl implements BookingService {
    ...
    public BookingDto approve(Long ownerId, Long bookingId, boolean approved) {
        Booking booking = bookingRepository.findById(bookingId)
                .orElseThrow(() -> new NotFoundObject("Бронирование не найдено"));
    ...
    public BookingDto findById(Long userId, Long bookingId) {
        ...
        if (!isBooker && !isOwner) {
            throw new NotFoundObject("Доступ запрещён");
        }
    ...
    public List<BookingDto> findAllByOwner(Long ownerId, BookingState state) {
        ...
        LocalDateTime now = LocalDateTime.now();
        return bookingMapper.toDtoList(getBookingsForOwner(ownerId, state, now));
    }
